function [op,opreport,opspec,opt] = load_flow_with_Sat(model,data,initState,alg,op1,rep)
...
opt.OptimizationOptions.TolX = 1e-6;
opt.OptimizationOptions.TolFun = 1e-6;
opt.OptimizationOptions.MaxIter = 200;
opt.OptimizationOptions.TolCon =  1e-6;

%% Perform the operating point search.


idxGov = find(~cellfun(@isempty,regexpi(bNames,'/S7/Integrator')) & ~cellfun(@isempty,regexpi(bNames,'General Governor')));
